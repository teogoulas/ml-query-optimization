...
select stuid from student ;
 select t1.stuid from student as t1 join has_pet as t2 on t1.stuid   =   t2.stuid join pets as t3 on t3.petid   =   t2.petid where t3.pettype   =   'cat';
select t1.name from visitor as t1 join visit as t2 on t1.id   =   t2.visitor_id join museum as t3 on t3.museum_id   =   t2.museum_id where t3.open_year  <  2009 ;
 select t1.name from visitor as t1 join visit as t2 on t1.id   =   t2.visitor_id join museum as t3 on t3.museum_id   =   t2.museum_id where t3.open_year  >  2011;
select t3.name from visitor as t1 join visit as t2 on t1.id   =   t2.visitor_id join museum as t3 on t3.museum_id   =   t2.museum_id where t3.open_year  <  2009 and t1.name  =   ( select t1.name from visitor as t1 join visit as t2 on t1.id   =   t2.visitor_id join museum as t3 on t3.museum_id   =   t2.museum_id where t3.open_year  <  2009 ;
 select t1.name from visitor as t1 join visit as t2 on t1.id   =   t2.visitor_id join museum as t3 on t3.museum_id   =   t2.museum_id where t3.open_year  >  2011 );
...
select name from country where code in  ( select code from country where governmentform ! =  'republic') ;
...
select property_name, room_count from properties where property_type_code   =   'house' ;
 select property_name, room_count from properties where property_type_code   =   'apartment' and room_count  >  1;
...
select count ( * )  from flights where destairport like '%asy%' and sourceairport like '%apg%' ;
 select count ( * )  from flights where destairport like '%asy%' and sourceairport like '%abr%';
...
select t1.country_id ,  t1.country_name from countries as t1 join car_makers as t2 on t1.country_id   =   t2.country group by t1.country_id having count ( * )   >  3 ;
 select t1.country_id ,  t1.country_name from countries as t1 join car_makers as t2 on t1.country_id   =   t2.country join model_list as t3 on t2.id   =   t3.maker where t3.model   =   'fiat';
...
select avg ( earnings )  as earning from poker_player ;
 select max ( earnings )  as earning from poker_player;
...
select series_name from tv_channel ;
 select series_name from tv_channel where language  =  'italian';
select id from tv_channel ;
 select channel from cartoon where directed_by  =  'ben jones';
...
select * from country as t1 join countrylanguage as t2 on t1.code   =   t2.countrycode where t2.language   =   'english' and isofficial   =   't' ;
 select * from country as t1 join countrylanguage as t2 on t1.code   =   t2.countrycode where t2.language   =   'dutch' and isofficial   =   't';
...
select code from country where governmentform ! =  'republic' ;
 select countrycode from countrylanguage where language   =   'english';
select countrycode, language from countrylanguage where countrycode in  ( select code from country where governmentform ! =  'republic' ;
 select countrycode from countrylanguage where language   =   'english' );
select code from country where continent  =  'north america' and code in  ( select code from country where governmentform ! =  'republic' ;
 select countrycode from countrylanguage where language   =   'english' );
...
select template_id from templates ;
 select template_id from documents;
select date_effective_to from templates where template_id in  (  select template_id from templates ;
 select template_id from documents );
select count ( * )  from templates where template_id in  ( select template_id from documents );
select count ( * )  from templates where template_id in  ( select template_id from templates ;
 select template_id from documents );
select template_id from templates ;
 select template_id from documents;
...
select t1.first_name, t1.last_name from students as t1 join addresses as t2 on t1.current_address_id   =   t2.address_id where t2.state_province_county   =   'northcarolina' ;
 select t3.first_name, t3.last_name from students as t3 join student_enrolment as t4 on t3.student_id   =   t4.student_id;
...
select count ( student_id )  from  ( select distinct student_id from friend ;
...
select name , result from battle ;
 select t1.name , t1.result  from battle as t1 join ship as t2 on t1.id  =  t2.lost_in_battle where t2.location  =  'english channel';
