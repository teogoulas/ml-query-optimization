...
select t1.state from college as t1 join tryout as t2 on t1.name   =   t2.cname where t2.ppos   =   'goalie' intersect select t1.state from college as t1 join tryout as t2 on t1.name   =   t2.cname where t2.ppos   =   'mid';
...
select count ( distinct student_id )  from  ( select student_id from student_course_registrations union select student_id from student_course_attendance );
select founder from manufacturers where name  =  'sony';
...
select sum ( hours )  from  ( select sum ( hours_played )  as hours from plays_games where stuid  =  1001 union select sum ( hoursperweek )  as hours from sportsinfo where stuid  =  1001 );
...
select t1.name from tourist_attractions as t1 join visitors as t2 join visits as t3 on t1.tourist_attraction_id   =   t3.tourist_attraction_id and t2.tourist_id   =   t3.tourist_id where t2.tourist_details   =   'vincent' intersect  select t1.name from tourist_attractions as t1 join visitors as t2 join visits as t3 on t1.tourist_attraction_id   =   t3.tourist_attraction_id and t2.tourist_id   =   t3.tourist_id where  t2.tourist_details   =   'vivian';
...
select population from county where county_name  =  'manning' union select avg ( population )  as population from county;
...
select * from  ( select t3.student_details, t2.date_to from addresses as t1 join people_addresses as t2 on t1.address_id  =  t2.address_id join students as t3 on t2.person_id  =  t3.student_id order by t2.date_from desc limit 3 )  order by date_to desc limit 1;
...
select count ( customer_id )  from customers;
select other_details from customers union select staff_details from staff;
...
select t1.name, t1.meter_300 from swimmer as t1 join record as t2 on t1.id   =   t2.swimmer_id where result   =   'win' intersect select t1.name, t1.meter_300  from swimmer as t1 join record as t2 on t1.id   =   t2.swimmer_id where result   =   'loss';
...
select t2.stuid from sportsinfo as t1 join student as t2 on t1.stuid   =   t2.stuid where t1.sportname   =   'football' intersect select t2.stuid from sportsinfo as t1 join student as t2 on t1.stuid   =   t2.stuid where t1.sportname   =   'lacrosse';
...
select attribute_data_type from attribute_definitions except select attribute_data_type from attribute_definitions where attribute_data_type  =  'bool';
select distinct t2.apt_number from apartment_bookings as t1 join apartments as t2 on t1.apt_id   =   t2.apt_id where t1.booking_status_code   =   'confirmed';
...
select country from competition except select country from competition where competition_type   =   'friendly';
select  ( * )  from  ( select country from competition except select country from competition where competition_type   =  'friendly' )  where country ! =  'slovenia';
...
select count ( * )  from faculty where sex  =  'm' union select count ( * )  from faculty where sex  =  'f';
select other_details from customers;
select customer_id from customers except select t2.other_details from policies as t1 join customers as t2 on t1.customer_id   =   t2.customer_id;
select other_details from customers except select t2.other_details from policies as t1 join customers as t2 on t1.customer_id   =   t2.customer_id;
select other_details from customers except select t2.other_details from policies as t1 join customers as t2 on t1.customer_id   =   t2.customer_id;
